...
def register_company(company: CompanyCreate, db: Session = Depends(get_db)):
    ...
    access_token = auth_service.login(db, username=company.username, password=company.password)
    return {"access_token": access_token, "token_type": "bearer"}


@router.post("/login", response_model=Token)
def login_for_access_token(form_data: LoginReq, db: Session = Depends(get_db)):
    access_token = auth_service.login(db, username=form_data.username, password=form_data.password)
    return {"access_token": access_token, "token_type": "bearer"}
