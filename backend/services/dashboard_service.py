...
class DashboardService:
    ...
    async def get_stats(self, current_user: User) -> Dict[str, Any]:
        ...
        # Empleados activos
        active_employees = total_employees_query.filter(Employee.is_active == True).count()
        
        # Total almacenes
        if current_user.role.name == 'admin':
            total_warehouses = self.db.query(Warehouse).count()
            active_warehouses = self.db.query(Warehouse).filter(
    ...
    async def get_recent_activities(self, current_user: User, limit: int = 10) -> Dict[str, Any]:
        ...
        for log in activities:
            activities_list.append({
                ...
                "photo_url": f"/uploads/faces/check_{log.id}.jpg" if log.distance else None
            })
    ...
    async def get_attendance_chart(self, current_user: User, days: int = 7) -> Dict[str, Any]:
        ...
        for i in range(days):
            ...
            checkins_query = self.db.query(AccessLog).filter(
                and_(
                    func.date(AccessLog.ts) == current_date,
                    AccessLog.event == 'in'
                )
            )
            ...
            if warehouse_filter is not None:
                ...
                checkouts_query = checkouts_query.join(Employee).filter(warehouse_filter)
    ...
    async def get_warehouse_chart(self, current_user: User) -> Dict[str, Any]:
        ...
        if current_user.role.name == 'admin':
            ...
            warehouses = self.db.query(Warehouse).filter(Warehouse.is_active == True).all()
        else:
