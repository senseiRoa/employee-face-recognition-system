...
class Company(Base):
    ...
    record_timezone = Column(
        String(50), nullable=True, default="UTC"
    )  # Timezone when record was created
...
class Role(Base):
    ...
    permissions = Column(
        JSON, nullable=True
    )  # e.g., {"warehouse_access": ["read", "write"], "employee_management": ["read"]}
...
class User(Base):
    ...
    record_timezone = Column(
        String(50), nullable=True, default="UTC"
    )  # Timezone when user record was created
    ...
    warehouse = relationship(
        "Warehouse", back_populates="users"
    )  # Changed from company
    role = relationship("Role", back_populates="users")
    user_login_logs = relationship(
        "UserLoginLog", back_populates="user", cascade="all, delete-orphan"
    )
...
class Employee(Base):
    ...
    )  # New field for employee identification
    ...
    record_timezone = Column(
        String(50), nullable=True, default="UTC"
    )  # Timezone when employee record was created
    ...
    access_logs = relationship(
        "AccessLog", back_populates="employee", cascade="all, delete-orphan"
    )
...
class FaceEncoding(Base):
    ...
    )  # New field for encoding quality

    created_at = Column(DateTime, default=datetime.datetime.utcnow, nullable=False)
    record_timezone = Column(
        String(50), nullable=True, default="UTC"
    )  # Timezone when face encoding was registered
...
class AccessLog(Base):
    ...
    )  # "entry", "exit", "denied", "unknown"
    access_method = Column(
        String(50), nullable=True
    )  # "face_recognition", "manual", "card"
    ...
    device_timezone = Column(
        String(50), nullable=True, default="UTC"
    )  # Device timezone when clock-in/out occurred
...
class UserLoginLog(Base):
    ...
    timestamp = Column(DateTime, default=datetime.datetime.utcnow, nullable=False)
    client_timezone = Column(
        String(50), nullable=True, default="UTC"
    )  # Client timezone when login occurred
...
class PasswordHistory(Base):
    ...
    record_timezone = Column(
        String(50), nullable=True, default="UTC"
    )  # Timezone when password change occurred
