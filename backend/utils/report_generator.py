...
class ReportGenerator:
    ...
    def generate_attendance_report_pdf(
        self, data: List[Dict[str, Any]], title: str, period: str
    ) -> bytes:
    ...
    def generate_attendance_report_excel(
        ...
        if data:
            for row_idx, row_data in enumerate(data, 6):
                ws.cell(
                    row=row_idx, column=1, value=row_data.get("employee_name", "N/A")
                )
                ws.cell(
                    row=row_idx, column=2, value=row_data.get("warehouse_name", "N/A")
                )
    ...
    def generate_attendance_report_csv(
        ...
        if data:
            ...
            df = pd.DataFrame(
                columns=["Empleado", "Almacén", "Entradas", "Último Acceso"]
            )
    ...
    def get_report_filename(
        self, report_type: str, format_type: str, timestamp: datetime = None
    ) -> str:
        ...
        format_extension = {"pdf": "pdf", "excel": "xlsx", "csv": "csv"}
